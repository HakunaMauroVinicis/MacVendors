...
pdf_path = r"C:\Users\Colaborador\Downloads\Mauro\IPV6-clientes\ClientesAracatuba.pdf"
...
with pdfplumber.open(pdf_path) as pdf:
    ...
    for page in pdf.pages:
        ...
        if text:
            ...
            for line in lines:
                print('linha:', linha)
                # Encontrar IP, MAC e Status na linha
                ...
                if ip_match and mac_match and status_match:
                    ...
                    # Assegurar que restam 5 partes (ID, Cliente, Login, Senha, Concentrador)
                    if len(parts) >= 5:
                        id_cliente = parts[0]
                        cliente = ' '.join(parts[1:-3])
                        login = parts[-3]
                        senha = parts[-2]
                        concentrador = parts[-1]
                        ...
                        data.append([id_cliente, cliente, login, senha, ip, concentrador, mac, status, marca])
                linha = linha + 1
        pagina = pagina + 1
...
excel_path = r"C:\Users\Colaborador\Downloads\Mauro\IPV6-clientes\ClientesAracatuba.xlsx"
